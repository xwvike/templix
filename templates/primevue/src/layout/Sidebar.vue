...
const { menus } = toRefs(routeStore)
const menusList = computed(() => menus.value)

const navigate = (menu: any) => {
  if (menu.type === 'index') {
    routeStore.toggleActive(menu, 'menu')
...
<template>
  ...
  >
    <div
      class="w-[18rem] rounded-xl border border-gray-200 dark:border-gray-700 h-full bg-slate-50 dark:bg-gray-800 flex flex-col"
    >
      <a class="px-6 py-4 flex gap-3.5 items-center flex-shrink-0">
        <Skeleton shape="circle" size="2rem"></Skeleton>
        <span class="font-bold dark:text-white">TEMPLIX</span>
      </a>
      ...
      <ul class="w-full h-full p-6 flex flex-col gap-2 scrollbar scrollbar-thin overflow-y-auto">
        <li @click="navigate(item)" v-for="item in menusList" class="cursor-pointer">
          <div
          ...
          >
            ...
            <i v-if="item.type === 'index'" class="pi pi-angle-down"></i>
          </div>

          <div v-if="item.children && item.active && item.type === 'index'" class="pl-1.5 mt-2 box-border w-full">
            <div
              v-for="item2 in item.children"
              @click.stop="navigate(item2)"
              class="text-base px-3 box-border py-2 flex items-center gap-2"
              :class="[item2.active ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400']"
            >
              <div
